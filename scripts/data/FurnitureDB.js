...
export const FURNITURE_DB = [
    ...
    { id: 'CafeTree', type: 'floor', layer: 1, name: '咖啡树', price: 100, color: 0x8e44ad, size: {x:2, y:2, z:1}, modelFile: 'CafeTree.glb', modelScale: 1.0, fixBottom: true },
    ...
    { 
        id: 'ChrismasTree', type: 'floor', layer: 1, name: '圣诞树', price: 100, color: 0x8d6e63, 
        size: {x:1.5, y:2, z:1.5}, modelFile: 'ChrismasTree2.glb', modelScale: 1.5, fixBottom: true,
        light: true, lightType: 'point', lightOffset: { x: 0, y: 1.8, z: 0 }
    },
    ...
    { 
        id: 'ChrismasTree_Small', type: 'small', layer: 2, name: '小圣诞树', price: 5, color: 0xffffff, 
        size: {x:0.3, y:0.3, z:0.3}, modelFile: 'ChrismasTree_Small.glb', modelScale: 1.0, fixBottom: true,
        light: true, lightType: 'point', lightOffset: { x: 0, y: 0.25, z: 0 }
    },
...
export function getFurnitureById(id) {
    return FURNITURE_DB.find(item => item.id === id);
}
...
export function getFurnitureByType(type) {
    return FURNITURE_DB.filter(item => item.type === type);
}

/**
 * 根据分类筛选家具 (用于商店)
 * @param {string} category - 'floor' | 'small' | 'wall' | 'decor'
 * @returns {Array}
 */
export function getFurnitureByCategory(category) {
    if (category === 'floor') {
        return FURNITURE_DB.filter(i => 
            i.type === 'floor' || i.type === 'functional'
        );
    }
    return FURNITURE_DB.filter(i => i.type === category);
}

export default FURNITURE_DB;
